...
export const CategorySelectBox = ({
  ...
  const handleClickOption = (e: React.MouseEvent<HTMLUListElement>) => {
    ...
    if (value) setClickedOption(value);
    setClickedBox(false);
  };
  ...
  return (
    ...
    >
      <CommonButton
        className={`px-3 py-1 rounded-2xl ${clickedOption === '카테고리' ? 'text-gray-400' : 'font-medium text-gray-600'}`}
      ...
      {clickedBox && (
        <ul
          className="absolute left-1/2 top-[38px] -translate-x-1/2 w-30 text-center divide-y border rounded-md"
          onClick={handleClickOption}
        >
          {data.map((item, i) => (
            <li key={`category-${i}`} data-option={item}>
              <CommonButton
                name={item.name}
